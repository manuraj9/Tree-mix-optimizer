...
############# deletion of middle row is not being handelled properly
...
#     # ✅ Skip empty rows and rows with missing or blank tree names
#     df = df.dropna(subset=['Target_Percentage', 'Spacing'], how='any')
#     df = df[df['Tree'].notna() & (df['Tree'].astype(str).str.strip() != '')]
...
if run_opt:
    ...
    # ✅ Reset index so iloc works as expected
    ...
    if not np.isclose(total_percentage, 100.0, atol=0.1):
        ...
        for i in range(len(df)):
            constraints.append({
                'type': 'ineq',
                'fun': lambda x, i=i: (x[i] / np.sum(x)) * 100 - (df.iloc[i]['Target_Percentage'] - target_percentage_tolerance)
            })
            constraints.append({
                'type': 'ineq',
                'fun': lambda x, i=i: (df.iloc[i]['Target_Percentage'] + target_percentage_tolerance) - (x[i] / np.sum(x)) * 100
            })
        ...
        if result.success:
            ...
            df['Trees'] = trees
            df['Final_Percentage'] = np.round((df['Trees'] / df['Trees'].sum()) * 100, 2)
            df['Area(Hectare)']=df['Trees']*df['Adjusted_Spacing']**2/107639.104
